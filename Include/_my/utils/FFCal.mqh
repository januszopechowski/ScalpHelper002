...
enum FFCalNewsClassShow {HighOnly=1,HighMedium =2,HighMediumLow=3};// High MighMedium HighMediumLow
...
public:
                     ...
                     FFCal(string symbol,ENUM_TIMEFRAMES period, int Past_Event_Display_Minutes2,FFCalNewsClassShow newsClassShow);
                    ~FFCal();
                    
int start2();

int  GetFirstMinutes(){  return Minutes[1];}
int  GetSecondMinutes(){  return Minutes[2];}
int  GetThirdMinutes(){  return Minutes[3];}


int  IsFirstEventNoMessage(){  int indexT = StringFind(event[1],Additional_news_per_settings_is_not_scheduled_Msg, 0);
return (indexT != -1);//  znalezione info o braku informacji
}

int  IsSecondEventNoMessage(){  int indexT = StringFind(event[2],Additional_news_per_settings_is_not_scheduled_Msg, 0);
return (indexT != -1);//  znalezione info o braku informacji
}
int  IsThirdEventNoMessage(){  int indexT = StringFind(event[3],Additional_news_per_settings_is_not_scheduled_Msg, 0);
return (indexT != -1);//  znalezione info o braku informacji
}

 

bool  IsFirstEventImpactHigh()
{  
   return (bool) (Impact[1]  == "High");//(!(Impact[1]  == "Low"    && Impact[5] == "Medium"));
}


string GetEventMessage(int indexT ){  return event[indexT];}
...
bool	   Ignore_Current_Symbol        ;// = false;
...
string  Part_1                        ;//= "Indicator Display Controls:";
bool    Indicator_On                  ;//= true;
int     Past_Event_Display_Minutes    ;//= 15;
int     Show_Impacts_H_HM_HML_123     ;//= 3;
bool    News_Panel_At_Chart_Right     ;//= true;


bool    News_Panel_In_Subwindow_1     ;//= true;
...
color   Panel_Title 	               ;//= Black;
color   Low_Impact_News_Color        ;// = C'000,125,029';
color   Medium_Impact_News_Color     ;// = MediumBlue;
color   High_Impact_News_Color       ;// = Crimson;
color   Bank_Holiday_Color           ;// = DarkOrchid;
color   Remarks_Color                ;// = DarkGray;
...
string  Part_4                        ;//= "Event #1 Alert Setting:";
 int	   Alert_Minutes_Before_Event_1  ;//= 0;
...
double   	   ExtMapBuffer0[10000];     // Contains (minutes until) each news event
...
int            Minutes[10], newsIdx, next, dispMins, Days, Hours, Mins;
...
string   	   myEvent,mainData[500][8], sData, csvoutput, sinceUntil, TimeStr;
//string    	   G,/*pair, cntry1, cntry2,*/ Title[8], Country[8], Impact[8],event[5],Color[5];
string    	   G,/*pair, cntry1, cntry2,*/ Title[10], Country[10], Impact[10],event[7],Color[7];
...
string Additional_news_per_settings_is_not_scheduled_Msg;
                    
  };
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
FFCal::FFCal(string symbol,ENUM_TIMEFRAMES period,int Past_Event_Display_Minutes2,FFCalNewsClassShow newsClassShow)
  {
  _symbol=symbol;
   _period=period;
   Init();
   ...
   Past_Event_Display_Minutes= Past_Event_Display_Minutes2;
   int class123=3;
   
   if(newsClassShow== FFCalNewsClassShow::HighOnly)
   {
      class123=1;
   }
   else if(newsClassShow== FFCalNewsClassShow::HighMedium)
   {
      class123=2;
   }   
   else if(newsClassShow== FFCalNewsClassShow::HighMediumLow)
   {
      class123=3;
   }      
   Show_Impacts_H_HM_HML_123  =class123;
...
const int READURL_BUFFER_SIZE   =100;
...
Indicator_On                  = true;
Show_Impacts_H_HM_HML_123     = 3;
News_Panel_At_Chart_Right     = true;
News_Panel_In_Subwindow_1     = false;//true;
Past_Event_Display_Minutes    = 600;
Display_Min_TF                = 1;
Display_Max_TF                = 43200;
TF_Choices                    = "1-5-15-30-60-240-1440-10080-43200";

__                            = "";
Part_2                        = "News Panel Settings:";
Panel_Title 	               = Black;
Low_Impact_News_Color         = C'000,125,029';
Medium_Impact_News_Color      = MediumBlue;
High_Impact_News_Color        = Crimson;
Bank_Holiday_Color            = DarkOrchid;
Remarks_Color                 = DarkGray;
Background_Color_             = White;
Show_Panel_Background         = true;

___                           = "";
Part_3                        = "Other Currency Settings:";
Show_USD_News                 = false;
Show_EUR_News                 = false;
Show_GBP_News                 = false;
Show_NZD_News                 = false;
...
____                          = "";
Part_4                        = "Event #1 Alert Setting:";
 Alert_Minutes_Before_Event_1  = 0; // no alerts
...
//int 	         xmlHandle, BoEvent, finalend, end, begin, minsTillNews, tmpMins, idxOfNext;
//int            Minutes[8], newsIdx, next, dispMins, Days, Hours, Mins;
//i,k,curX,curY,W,Box,x1,x2,index;
WebUpdateFreq = 240; // 240 Minutes between web updates to not overload FF server
TxtSize       = 7;
TitleSpacer   = 6; 
EventSpacer   = 4; 
PrevMinute    = -1;
...
PrevTF        = 0;
UpdateRateSec = 10;
sUrl = "http://www.forexfactory.com/ff_calendar_thisweek.xml"; //original
//myEvent,mainData[200][7], sData, csvoutput, sinceUntil, TimeStr;
//G,/*pair, cntry1, cntry2,*/ Title[8], Country[8], Impact[8],event[5],Color[5];
string a[7] ={ "<title>", "<country>", "<date><![CDATA[","<time><![CDATA[","<impact><![CDATA[", "<forecast><![CDATA[", "<previous><![CDATA[" };
string b[7] ={ "</title>", "</country>", "]]></date>", "]]></time>","]]></impact>", "]]></forecast>", "]]></previous>" };
for (i=0;i<7;++i)
{
sTags[i] = a[i];//{ "<title>", "<country>", "<date><![CDATA[","<time><![CDATA[","<impact><![CDATA[", "<forecast><![CDATA[", "<previous><![CDATA[" };
eTags[i] =b[i];
}
Text_Style = "Tahoma";
//header;
...
News3   = "z[News] News3";
News4   = "z[News] News4";
Sponsor = "z[News] Sponsor";

// importand
Additional_news_per_settings_is_not_scheduled_Msg= "Additional news per settings is not scheduled";
...
int FFCal::init2()
   ...
   if ( !IsDllsAllowed() ) 
      {
      Alert(_symbol," ",_period,", FFCal: Allow DLL Imports");
      }
   ...
   for (k=(int)calendardate;k>=(int)(calendardate-6048000);k=k-604800)
      {
...
int FFCal::start2()
   ...
   if((!Indicator_On) || ((_period < Display_Min_TF) || (_period > Display_Max_TF)))
      {
      if (!Deinitialized)  {deinit2(); Deinitialized = true;}
 ...
 //     Print("PrevTF==_period");
	...
	if(xmlHandle>=0)
	   {
	   int size = (int)FileSize(xmlHandle);
		...
		for (i=0; i < 7; i++)
		   {
			...
			if (next == -1) continue;
			else
			   {
				...
				if (end > begin && end != -1)
				   {
				    someData=StringSubstr(myEvent, begin, end - begin);
				   // Print(" someData ", someData);
				      mainData[newsIdx][i] = someData;
				   }
			   }
		   }//End "for" loop
      someData = mainData[newsIdx][COUNTRY];
      someFlag=IsNewsCurrency((_symbol),someData);
...
//		else if (!(StringSubstr(mainData[newsIdx][TITLE],0,8)== "Daylight"))
		...
		if (!skip)
		   {
			...
			someData=mainData[newsIdx][DATE];
			someData2= mainData[newsIdx][TIME];
			newsTime = MakeDateTime(someData, someData2);
			minsTillNews = (int)((newsTime - TimeGMT()) / 60);
			//Print("newsTime ",newsTime, " ",someData," ",someData2," minsTillNews ",minsTillNews); 
         ...
         //
			...
			// Print("ExtMapBuffer0[newsIdx] idx : ",newsIdx," , mins : ",ExtMapBuffer0[newsIdx]);
         ...
         if(Past_Event_Display_Minutes > 0)
            ...
            if (ExtMapBuffer0[idxOfNext] < -Past_Event_Display_Minutes)
               ...
               Country[1] 	= mainData[idxOfNext+1][COUNTRY];
	            ...
	            Minutes[1] 	= (int)ExtMapBuffer0[idxOfNext+1]; 
	            i = idxOfNext+2;   
	            }         
	         }
         ...
         if ((FLAG_1_done) && (!FLAG_2_done))
		      ...
		      if (Minutes[2] <= 0) 	
               ...
               Country[2] 	= mainData[idxOfNext+1][COUNTRY];
	            ...
	            Minutes[2] 	= (int)ExtMapBuffer0[idxOfNext+1]; 
		         }		   	
   ...
   if (dispMins < 60) {TimeStr = (string)dispMins + " mins ";}
	else // time is 60 minutes or more
	   {
		Hours = (int)MathRound(dispMins / 60);
	   Mins = dispMins % 60;
	   if (Hours < 24) // less than a day: show hours and minutes
		   {
			TimeStr = (string)Hours + " hrs " + (string)Mins + " mins ";
		   }
		else // days, hours, and minutes
		  	{
		  	Days = (int)MathRound(Hours / 24);
		 	Hours = Hours % 24;
		  	TimeStr = (string)Days + " days " + (string)Hours + " hrs " + (string)Mins + " mins ";
		  	}
	   }
   index = StringFind(TimeStr+sinceUntil+Country[4], "since  mins", 0);
   TxtColorNews = (color)ImpactToColor(Impact[4]);
	if((index != -1) || (FLAG_4_none))
	   {
	   event[4] = "(4)  "+Additional_news_per_settings_is_not_scheduled_Msg+" "+(string)index;
	   Color[4] = (string)Remarks_Color;
	   }
	else
	   {
	   event[4] = TimeStr + sinceUntil + Country[4] + ": " + Title[4];
	   Color[4] = (string)TxtColorNews;
		}
   ...
   TxtColorNews = (color)ImpactToColor(Impact[3]);
	if((index != -1) || (FLAG_3_none))
		{
		event[3] = "(3)  "+Additional_news_per_settings_is_not_scheduled_Msg;
		Color[3] = (string)Remarks_Color;
		}
	else
	   {
	   event[3] = TimeStr + sinceUntil + Country[3] + ": " + Title[3];
	   Color[3] = (string)TxtColorNews;
		}
   ...
   TxtColorNews = (color)ImpactToColor(Impact[2]);
	if((index != -1) || (FLAG_2_none))
		{	
		event[2] = "(2)  "+Additional_news_per_settings_is_not_scheduled_Msg;
		Color[2] = (string)Remarks_Color;
      }
	else
	   {
	   event[2] = TimeStr + sinceUntil + Country[2] + ": " + Title[2];
	   Color[2] = (string)TxtColorNews;
		}
   ...
   curY = curY + TxtSize + EventSpacer;
	ObjectSetText(News4, event[4], TxtSize, Text_Style, (color)Color[4]);
   ...
   curY = curY + TxtSize + EventSpacer;
	ObjectSetText(News3, event[3], TxtSize, Text_Style, (color)Color[3]);
   ...
   curY = curY + TxtSize + EventSpacer;
	ObjectSetText(News2, event[2], TxtSize, Text_Style, (color)Color[2]);
   ...
   curY = curY + TxtSize + TitleSpacer;
	ObjectSetText(News1, event[1], TxtSize, Text_Style, (color)Color[1]);
	...
	if(Alert_Minutes_Before_Event_1!=0)
	   ...
	   if(Alert_Minutes_Before_Event_1<Minutes[1])
         {
	      header = "RELEASES  ("+(string)Alert_Minutes_Before_Event_1+" Min. Alert Pending):";
   	   }
...
void FFCal::DownLoadWebPageToFile(string url = "http://www.forexfactory.com/ff_calendar_thisweek.xml")
   {
   ...
   while (true)
      ...
      if (read[0] > 0)NEWS = NEWS + strThisRead;
         ...
         FileClose(xmlHandle);
		   ...
		   if (end == -1)
		      {
		      Alert(_period," ",_period,", FFCal Error: File download incomplete!");
		      return;
		      }
...
//+-----------------------------------------------------------------------------------------------+
//deVries:
string FFCal::PadString(string toBePadded, string paddingChar, int paddingLength)
   {
...
bool FFCal::IsNewsCurrency(string cSymbol, string fSymbol)
   {
...
double FFCal::ImpactToColor (string impact)
   {
...
//+-----------------------------------------------------------------------------------------------+
datetime FFCal::MakeDateTime(string strDate, string strTime)  //not string now datetime
   {
	...
	datetime newsevent = StringToTime(strYear+ "." + strMonth + "." +
	   strDay)+(int)(nHour24*3600)+ (int)(StringToInteger(strMinute)*60);
	return(newsevent);
   }
